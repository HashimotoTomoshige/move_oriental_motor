...
def resCallback(res):
    ...
    # if (res.slave_id == 2 and res.func_code == 3):
...
def main():
    ...
    pub = rospy.Publisher("/om_modbusRTU_1/om_query1", om_query,
                          queue_size=1)  # OMノードに送信するための定義

    rospy.Subscriber("/om_modbusRTU_1/om_response1", om_response,
                     resCallback)  # レスポンスのコールバック定義
    rospy.Subscriber("/om_modbusRTU_1/om_state1", om_state, stateCallback)  # レスポンスのコールバック定義
    msg = om_query()  # ノードで定義されたメッセージを使用
    time.sleep(1)

    # 読み込み(運転データNo.0の位置)
    # 500で1回転
    msg.slave_id = 0x02     # 号機選択(Hex): 1号機
    msg.func_code = 0       # ファンクションコード選択: 0(Read)
    msg.read_addr = 256    # 先頭アドレス選択(Dec): 運転データNo.0の位置
